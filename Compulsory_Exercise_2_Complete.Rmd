...
&- 0.168 \cdot \texttt{F.Undergrad} + 0.89 \cdot \texttt{Room.Board}-0.25 \cdot \texttt{Personal} +12.81 \cdot \texttt{PhD} + 23.65 \cdot \texttt{Terminal} \\
...
print(k_means_plot)
```
From the above plot where tissue type is denoted by color, and cluster from the K-means algorithm is denoted as different symbols, we see from visual investigation that the error rate for K-means when it comes to seperating the two tissue types is $0\%$.